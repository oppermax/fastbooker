...
/**
 * Split a time block into chunks of maximum size
 * This is required by Affluences API - bookings cannot exceed a certain duration
 * Different libraries may have different maximum booking durations
 */
 ...
 * Blocks are automatically split to respect maximum booking limit
 */
function findAvailableBlocks(seat, startMinutes, endMinutes, maxChunkMinutes = 240) {
  const blocks = [];
  ...
  for (const hour of sortedHours) {
    ...
    if (hour.places_available === 0) {
      ...
      if (currentBlockStart !== null) {
        ...
        // Split into chunks based on max chunk size
        const chunks = splitBlockIntoMaxChunks({ start: currentBlockStart, end: blockEnd }, maxChunkMinutes);
        blocks.push(...chunks);
 ...
 */
function scoreCombination(combination, targetDuration, maxChunkMinutes = 240) {
  let score = 0;
  ...
  // Penalty for very short bookings (but not max-length chunks which are good)
  for (const segment of combination) {
    ...
    // Max-length blocks or close to it are ideal
    if (duration < 60) {
      score += (60 - duration) * 0.5;
    }
  }

  // Bonus for longer bookings (2 hours to max chunk size are ideal)
  const avgDuration = totalCovered / combination.length;
  if (avgDuration >= 120 && avgDuration <= maxChunkMinutes) {
    score -= 30; // 2 hours to max chunk size blocks are perfect
  }
 ...
 */
export function findOptimalSeatCombinations(seats, startTime, endTime, maxOptions = 5, maxChunkMinutes = 240) {
  ...
  const seatAvailability = seats.map(seat => ({
    seat,
    blocks: findAvailableBlocks(seat, startMinutes, endMinutes, maxChunkMinutes)
  ...
  const scoredCombinations = combinations
    .map(combo => ({
      combination: combo,
      score: scoreCombination(combo, targetDuration, maxChunkMinutes),
