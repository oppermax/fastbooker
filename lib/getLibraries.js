...
export default async function getLibraries() {
  // Cache libraries for 10 minutes since they rarely change
  return getCached('all_libraries', async () => {
    const allResults = [];
    let currentPage = 0;
  ...
  }, { ttl: 2 * 60 * 1000 }); // Cache for 2 minutes (reduced for real-time open/closed status)
}
